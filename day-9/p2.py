...
mdf1=pd.merge(df1,df3,on="CustomerID")
mdf2=pd.merge(mdf1,df2,on="ProductID")
mdff=pd.merge(mdf2,df4,on="OrderID")
print("Complete Sales Report")
csr=mdff.loc[:,["OrderID","CustomerName","City","ProductName","Category","Quantity","PaymentAmount"]]
print(csr.to_string(index=False,header=False))
print()
print("Electronics Orders")
eo=mdff.loc[mdff["Category"]=="Electronics",["OrderID","CustomerName","ProductName","PaymentAmount"]]
print(eo.to_string(index=False,header=False))
print()
print("Second and Third Orders")
sto=mdff.loc[:,["OrderID","CustomerName","ProductName","PaymentAmount"]]
stof=sto.iloc[1:3]
print(stof.to_string(index=False,header=False))
print()
print("Category Revenue Report")
crr=mdff.groupby("Category")["PaymentAmount"].sum()
crr=crr.sort_values(ascending=False)
print(crr.to_string(header=False))
print()
print("Highest Revenue Category")
print(crr.iloc[0:1].to_string(header=False))
print()
print("Highest Spending Customer")
customer_s=mdff.groupby("CustomerName")["PaymentAmount"].sum()
customer_s=customer_s.sort_values(ascending=False)
print(customer_s.iloc[0:1].to_string(header=False))
