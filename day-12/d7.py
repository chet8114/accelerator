...
res=df.groupby("StoreCode")["Sales"].mean().sort_index()
print("Pandas Aggregated Performance Metrics")
for store,avg in res.items():
    print(store,f"{avg:.1f}")
print()
p_df=pl.read_csv("sales.csv")
p_res=(p_df.lazy().group_by("StoreCode").agg(pl.col("Sales").mean().alias("Sales")).sort("StoreCode").collect())
print("\nPolars Aggregated Performance Metrics")
for row in p_res.iter_rows():
    print(*row)
