...
unop_m=df[["Status","Priority"]].memory_usage(deep=True,index=False).sum()
print(f"Unoptimized String Memory: {unop_m} Bytes")
df["Status"]=df["Status"].astype("category")
df["Priority"]=df["Priority"].astype("category")
opt_m=(df[["Status","Priority"]].memory_usage(deep=True,index=False).sum())
print(f"Optimized Category Memory: {opt_m} Bytes")
print("Status Codes Mapping")
for code, value in enumerate(df["Status"].cat.categories):
    print(f"{value} : {code}")