...
if r <= 0:
    ...
    if c <= 0:
        ...
        marks = np.array([int(input()) for i in range(r * c)])
        valid = True
        for mark in marks:
        ...
        if not valid:
            ...
            matrix = marks.reshape(r, c)
            student_totals = np.sum(matrix, axis=1)
            student_avg = np.round(np.mean(matrix, axis=1), 2)
            subject_avg = np.round(np.mean(matrix, axis=0), 2)
            overall_avg = np.round(np.mean(matrix), 2)
            top = np.argmax(student_totals)
            print("Student Totals:")
            for i in range(r):
                print(f"Student {i+1} : {student_totals[i]}")
            print()
            ...
            for i in range(c):
                print(f"Subject {i+1} : {subject_avg[i]}")
            print()
            print(f"Overall Class Average : {overall_avg}")
            print(f"Top Scorer : Student {top+1}")